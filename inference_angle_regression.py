...
from utils.utils import generate_synthetic_pairs_regression
...
def main():
    ...
    parser.add_argument('--model_name', type=str, default='./pretrained/rotation_angle_regressor_custom_50_epoch.pth',
    ...
    if args.image1 is not None and args.image2 is not None:
        ...
        val_image_pairs, val_angles = generate_synthetic_pairs_regression(val_dataset)
        ...
        true_label = val_angles[args.test_index]
        print(f"True Rotation Angle: {true_label:.2f} degrees")

    # Predict the rotation angle
    with torch.no_grad():
        predicted_angle = model(original_image, rotated_image).item()
        print(f"Predicted Rotation Angle: {predicted_angle:.2f} degrees")

    # Correct the rotated image using the predicted rotation angle.
    corrected_image = transforms.functional.rotate(rotated_image, -predicted_angle)  # Negative input predicted angle
    ...
    title_name = "Corrected image by predicted angle of " + f"{predicted_angle: .2f}" + degree
